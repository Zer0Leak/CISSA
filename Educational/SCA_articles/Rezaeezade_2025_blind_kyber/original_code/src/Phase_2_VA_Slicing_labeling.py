...
# Function to perform majority voting on a single row
...
def linge_label(traces, poi, num_bits):
    sorted_index = np.argsort(traces[:, poi], axis=0)  # sort in ascending order
...
def LinearRegression_VarianceAnalysis(traces_m, traces_y, num_bits, variance_noise):
    ...
    Alpha_m = np.sqrt((variance_trace_m - variance_noise) / (num_bits / 4))  # todo: why got plus minus?
    ...
    # Apply majority voting to each row. This one should be uncommented if I want to use multiple PoI for traditional
    # techniques
    # majority_m = np.apply_along_axis(majority_vote, axis=1, arr=HW_m)
    # majority_y = np.apply_along_axis(majority_vote, axis=1, arr=HW_y)
    # Y_pred = np.array([list(majority_m), list(majority_y)])
    # Y_pred = np.array([list(HW_m), list(HW_y)])
    return HW_m, HW_y
...
def LinearRegression_VarianceAnalysis_Ascon(traces_m1, traces_m2, traces_y, num_bits, var_noise=0.001):
    ...
    variance_trace_y = np.var(traces_y)
    Alpha_m1 = np.sqrt((variance_trace_m1 - var_noise) / (num_bits / 4))  # todo: why got plus minus?
    Alpha_m2 = np.sqrt((variance_trace_m2 - var_noise) / (num_bits / 4))  # todo: why got plus minus?
