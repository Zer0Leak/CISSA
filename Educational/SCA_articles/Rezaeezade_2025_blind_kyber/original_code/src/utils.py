...
def load_chipwhisperer_blind(chipwhisper_folder, byte=0, sync=True, noise_std=0.0):
    if sync:
        X = np.load(chipwhisper_folder + 'traces.npy')[:10000]
        print("Synchronized dataset is loaded!")
    else:
        X = np.load(chipwhisper_folder + 'Desync_traces_10.npy')[:10000]
    ...
    # labels are checked with the constructed ones.
    labels = np.load(chipwhisper_folder + 'labels.npy')[:10000]
    HW_Y = np.asarray(calculate_HW(labels))
    # The key is checked. 16 bytes key with bytes all the same for 10000 traces.
...
def load_kyber_blind(saving_dataset_root, dataset_name, traces_no, window, load_reduced):
    overlap = 0.5
    ...
    if os.path.exists(file_path):
        ...
        # First 2 bytes are secret key, next 2 bytes are input, next 2 bytes are output
...
def load_ascon_blind(root_data, dataset_name, num_traces, target=0):
    import trsfile
    ...
    samples = np.zeros((num_traces, ns), dtype=np.float64)
    ...
    print("Good key is: ", keys[100001][target])
    correct_key = keys[100001][target]

    key0 = keys[100000:, target]
    # key1 = keys[100000:, target+8]
    ...
    return samples[100000:, :], Y, P1, P2, correct_key
...
def two_dim_hw_to_joint(two_dim_hw):
    # m * num_classes + y
    # print("two_dim_hw shape: ", two_dim_hw.shape)
...
def joint_hw_to_three_dim(joint_hw):
    # print("classes in joint_hw_to_two_dim: ", 9*9*9)
    p1 = joint_hw // 81
    p2 = (joint_hw % 81) // 9
    y = joint_hw % 9
    return p1, p2, y
...
def total_accuracy(actual, empirical):
    ...
    for i in range(num_examples):
        if empirical[i] == actual[i]:
            accuracy += 1
    # print("all corrected clustered: ", accuracy, f" from {num_examples} traces")
    # print("total_accuracy: ", accuracy/num_examples * 100)
...
def customized_accuracy_ascon(predicted_hw, actual):
    ...
    return total_acc, class_acc_m1, class_acc_m2, class_acc_y
